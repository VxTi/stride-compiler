...
int stride::ast::parse_function_call(ast_token_set_t &token_set, cursor_t index, Node &root)
{
    ...
    if ( function_call_arguments->token_count > 0 )
    {
        ...
        for ( int i = 0, expression_length = 1;
              i < function_call_arguments->token_count &&
              expression_length > 0; )
        {
            parse_expression(token_set, index + i, expression_length, *function_call_params_node);
            expression_length = distance_next_token_outside_block(token_set, i, TOKEN_COMMA);
